...
class DXFViewer(QMainWindow):
    ...
    def initUI(self):
        ...
        self.canvas.mpl_connect('button_press_event', self.on_canvas_click)  # 마우스 클릭 이벤트 연결
    ...
    def visualize_entities(self):
        ...
        for idx, (entity, visible) in enumerate(zip(self.entities, self.entity_visibility)):
            if not visible:
                continue
            color = 'blue' if idx == self.selected_entity_index else 'black'
            if entity.dxftype() == 'LWPOLYLINE':
